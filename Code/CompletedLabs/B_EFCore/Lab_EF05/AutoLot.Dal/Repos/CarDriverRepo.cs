...
public class CarDriverRepo : BaseRepo<CarDriver>, ICarDriverRepo
{
    ...
    public override CarDriver Find(
        int? id) => id == null ? null : BuildBaseQuery().IgnoreQueryFilters().FirstOrDefault(cd => cd.Id == id.Value);

    public override CarDriver FindAsNoTracking(
        int id) => BuildBaseQuery().AsNoTracking().FirstOrDefault(cd => cd.Id == id);

    public override CarDriver FindIgnoreQueryFilters(
        int id) => BuildBaseQuery().IgnoreQueryFilters().FirstOrDefault(cd => cd.Id == id);
}