...
public class CarRepo : BaseRepo<Car>, ICarRepo
{
    ...
    public override Car Find(
        int? id) => id == null ? null : BuildBaseQuery().IgnoreQueryFilters().FirstOrDefault(c => c.Id == id.Value);

    public override Car FindAsNoTracking(
        int id) => BuildBaseQuery().AsNoTracking().FirstOrDefault(c => c.Id == id);

    public override Car FindIgnoreQueryFilters(
        int id) => BuildBaseQuery().IgnoreQueryFilters().FirstOrDefault(c => c.Id == id);

    public IList<Car> GetAllByAsList(
        int makeId) => GetAllByAsQueryable(makeId).ToList();

    // Returns deferred IQueryable — DbContext must remain alive until materialized.
    public IQueryable<Car> GetAllByAsQueryable(
        int makeId) => BuildBaseQuery().Where(c => c.MakeId == makeId);

    public string GetPetName(
        int id)
    {
        var outParam = new SqlParameter("@petName", SqlDbType.NVarChar, 50) { Direction = ParameterDirection.Output };
        ExecuteParameterizedQuery("EXEC GetPetName @id, @petName OUT", [new SqlParameter("@id", id), outParam]);
        return outParam.Value?.ToString();
    }

    public int SetAllDrivableCarsColorAndMakeId(
        string color,
        int makeId) => ExecuteBulkUpdate(c => c.IsDrivable,
